...
const formSchema = z.object({
  ...
  value: z.string().min(4).regex(/^#/, {
    message: "Cor precisa ser um HEX válido.",
  }),
});
...
export const ColorForm: React.FC<IColorFormProps> = ({ initialData }) => {
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error(
        "Certifique-se de remover todas os produtos que utilizam essa cor primeiro."
      );
    } finally {
      setLoading(false);
      setOpen(false);
